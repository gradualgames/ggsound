...
;
; Modified by Derek Andrews to work on nesasm. Nesasm has no alignment
; directive so I'm just placing it an explicit address with the correct
; alignment. Secondly, nesasm cannot generate zp instructions so I use
; .db to output the correct instructions.
...
get_tv_system:
  ldx #0
  ldy #0
  .db $A5,low(nmis) ;lda nmis
.nmiwait1:
  .db $C5,low(nmis) ;cmp nmis
  beq .nmiwait1
  .db $A5,low(nmis) ;lda nmis
